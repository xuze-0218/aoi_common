...
namespace aoi_common.Services
{
    ...
    public class DetectionLogicService : IDetectionLogicService
    {
        ...
        public DetectionResult ProcessPlcData(string rawPlcData)
        {
            ...
            if (_config.GetInt("系统设置", "ModuStatus") == 0)
            {
        ...
        private void ProcessCalibration(DetectionResult res, List<string> logs)
        {
            res.ItemCode = 100;
            res.Message = "成功进入标定！";
            // 从 "相机参数" 模块获取配置，你可以根据你的实际 JSON 结构修改模块名
            res.Exposure = _config.GetFloat("相机参数", "标定曝光");
            res.Gain = _config.GetFloat("相机参数", "标定增益");
            res.IsSuccess = true;
        ...
        private void ProcessDetection(DetectionResult res, List<string> logs, DetectCode dc)
        {
            ...
            bool bigTrans = _config.GetBool("全局设置", "bigTrans");
            bool doubleTrans = _config.GetBool("全局设置", "doubleTrans");

            if (pType == 1 || pType == 2) pType = !bigTrans ? 1 : 2;
            else if (pType == 3 || pType == 4) pType = !doubleTrans ? 3 : 4;

            bool plcOrNot = _config.GetBool("全局设置", "plcOrNot");
            int finalType = (plcOrNot && pType != 0)
                            ? _config.GetInt("产品参数", "产品类型")
                            : pType;

            // 默认参数
            res.Exposure = _config.GetFloat("相机参数", "默认曝光");
            res.Gain = _config.GetFloat("相机参数", "默认增益");
            ...
            switch (finalType)
            {
                ...
                case 4:
                    ...
                    res.Exposure = _config.GetFloat("相机参数", "胶条曝光");
                    res.Gain = _config.GetFloat("相机参数", "胶条增益");
                    res.IsSuccess = true;
                    break;
        ...
        {
            if (_config.GetBool("全局设置", "ignore")) return true;
            string ignoreOne = _config.GetString("全局设置", "ignoreOne");
